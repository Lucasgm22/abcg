...
void Window::onCreate() {
  ...
  // Randomly pick x value
  std::uniform_real_distribution<float> realDistribution(-2.0f, 2.0f);
  m_P.x = realDistribution(m_randomEngine);
  m_P.y = yFunction(m_P.x);
}
...
void Window::onPaint() {
  ...
  // Randomly pick a pair of x value
  std::uniform_real_distribution<float> realDistribution(-2.0f, 2.0f);
  m_P.x = realDistribution(m_randomEngine);
  m_P.y = yFunction(m_P.x);
...
float Window::yFunction(float x) {
  std::discrete_distribution<int> intDistribution({50, 50});
  auto const top{intDistribution(m_randomEngine)};
  if (top == 1) {
    return (sqrt(1 - pow((abs(x*4) - 1), 2))) / 3;
  } else {
    return (acos(1 - abs(x*4)) - M_PI) / 4;
  }

...
void Window::onPaint() {
  ...
  std::discrete_distribution<int> intDistribution({m_A, m_B, m_C});
...
void Window::onPaintUI() {
...

  {
    ...
    auto total = m_A + m_B + m_C;
    m_A = 100 * (m_A/ total);
    m_B = 100 * (m_B / total);
    m_C = 100 * (m_C / total);
    ImGui::SliderFloat("A", &m_A, 0, 100, "%.2f", ImGuiSliderFlags_NoInput);
    ImGui::SliderFloat("B", &m_B, 0, 100, "%.2f", ImGuiSliderFlags_NoInput);
    ImGui::SliderFloat("C", &m_C, 0, 100, "%.2f", ImGuiSliderFlags_NoInput);

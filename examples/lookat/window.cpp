...
void Window::onEvent(SDL_Event const &event) {
  ...
  if (event.type == SDL_KEYUP) {
    ...
    if (event.key.keysym.sym == SDLK_s && m_dollySpeed < 0)
      m_dollySpeed = 0.0f;
    if (event.key.keysym.sym == SDLK_UP && m_upSpeed > 0)
      m_upSpeed = 0.0f;
    if (event.key.keysym.sym == SDLK_DOWN && m_upSpeed < 0)
      m_upSpeed = 0.0f;
    if (event.key.keysym.sym == SDLK_a && m_panSpeed < 0)
      m_panSpeed = 0.0f;
    if (event.key.keysym.sym == SDLK_d && m_panSpeed > 0)
      m_panSpeed = 0.0f;
    if (event.key.keysym.sym == SDLK_LEFT && m_rotateSpeed < 0) 
      m_rotateSpeed = 0.0f;
    if (event.key.keysym.sym == SDLK_RIGHT && m_rotateSpeed > 0) 
      m_rotateSpeed = 0.0f;
    if (event.key.keysym.sym == SDLK_q && m_truckSpeed < 0)
      m_truckSpeed = 0.0f;
    if (event.key.keysym.sym == SDLK_e && m_truckSpeed > 0)
      m_truckSpeed = 0.0f;
    if (event.key.keysym.sym == SDLK_LSHIFT && m_run > 1) 
      m_run = 1.0f;
  }
}
...
void Window::onUpdate() {
  ...
  m_camera.dolly(m_dollySpeed * m_run * deltaTime);
  m_camera.truck(m_truckSpeed * m_run * deltaTime);
  m_camera.pan(m_panSpeed * deltaTime);
  m_camera.up(m_upSpeed * m_run * deltaTime);
